...
module.exports = function (cfg) {
  ...
  return function authenticateUser(credentials) {
    ...
    return db
      .getAsync(id)
      .spread(function hash (doc) {
        return pbkdf2(credentials.password, doc.salt, doc.iterations, 20)
          .then(function check (key) {
            if(doc.derived_key === key.toString('hex')) {
              return jwtSign({ user: doc.name }, privateKey, { expiresInMinutes: expiration });
            } else {
